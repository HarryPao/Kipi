...
include("kipi_pdoInc.php");
    ...
    if($_SESSION['account'] == $host){
        if(isset($_GET['id'])  && isset($_POST['content'])){
            ...
            $sth = $dbh->prepare('UPDATE post SET content =  ? WHERE id = ? ');
            $sth->execute(array($_POST['content'],(int)$_GET['id']));

            echo "<script type='text/javascript'>";
            echo "alert('The post has been editted!');";
            echo "</script>";
        ...
        echo "<script type='text/javascript'>";
        echo "alert('You have no permission to edit this post!');";
        echo "</script>";
        echo '<meta http-equiv=REFRESH CONTENT=0;url=kipi_post.php?id='.$_SESSION['tmp'].'>'; 
    }
...
<body>
    <div class="container">
        ...
        <form role="form" action="kipi_update_post.php?id=<?php echo (int)$_GET['id'];?>" method="POST">
            <div class="form-group">

                
                <input type="text" class="form-control" id="content" placeholder="new content" name="content"  > 
